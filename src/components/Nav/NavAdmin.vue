<template>
    <header class="text-gray-400 bg-gray-900 body-font sticky top-0 z-10">
        ...
        <nav :class="{ 'hidden': !menuOpen, 'flex': menuOpen }"
            class="md:hidden flex-col md:flex-row items-center text-base justify-center w-full md:w-auto bg-gray-900 p-5 md:p-0">
            <router-link to="/" @click="toggleMenu" class="mr-5 hover:text-white cursor-pointer">Inicio</router-link>
            <router-link to="/productos" @click="toggleMenu"
                class="mr-5 hover:text-white cursor-pointer">Productos</router-link>
            <router-link to="/contacto" @click="toggleMenu"
                class="mr-5 hover:text-white cursor-pointer">Contacto</router-link>
        </nav>
    </header>
</template>
